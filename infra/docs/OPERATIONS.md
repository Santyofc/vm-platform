...
## 1. Verificación del servidor EC2
Puedes corroborar rápidamente que el sistema principal está respondiendo:
```bash
# Validacion en el túnel interno directamente:
curl http://127.0.0.1:3000

# Validacion mediante IP publica llegando al proxy Nginx de AWS proxy
curl http://3.137.182.156

# Validar estado a nivel docker:
docker ps

# Validar estado el servicio envuelto en Systemd
sudo systemctl status vm-platform
```

## 2. Desplegar Nuevos Cambios (Actualizar)
...
## 3. Realizar un Rollback (Volver atrás)
...
## 5. Revisar Logs del Sistema Next.js
...
## 7. Validar Estado de Healthcheck
...
## 8. Mantenimiento y Certificados
